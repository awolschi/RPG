...
bool SaveGameManager::SaveGame(const std::shared_ptr<Player>& player, int slot, int areaIndex, const ReligionSystem& religion)
{
    ...
    try
    {
        ...
        auto writeName = [&](const auto& ptr) {
            file << (ptr ? ptr->name : "none") << "\n";
        };
        writeName(eq.weapon); writeName(eq.offhand);
        writeName(eq.helmet); writeName(eq.chest);
        writeName(eq.gloves); writeName(eq.pants);
        writeName(eq.boots);  writeName(eq.ring1);
        writeName(eq.ring2);  writeName(eq.amulet);
...
std::shared_ptr<Player> SaveGameManager::LoadGame(int slot, int& outAreaIndex, ReligionSystem& outReligion)
{
    ...
    try
    {
        ...
        std::vector<std::tuple<std::string, int, int, int, int>> savedSkills;
        for (size_t i = 0; i < skillCount; ++i)
        {
            ...
            savedSkills.emplace_back(skname, std::stoi(sklvl), std::stoi(skxp), std::stoi(skcd), std::stoi(skdmg));
        }

        for (const auto& [sname, slvl, sxp, scd, sdmg] : savedSkills)
        {
            auto sk = player->GetSkills().GetSkillByName(sname);
            if (sk)
            {
                sk->level = slvl;
                sk->experience = sxp;
                sk->currentCooldown = scd;
                sk->baseDamage = sdmg;
            }
        }
